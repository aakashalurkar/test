...
Open [http://54.219.175.186:3000](http://54.219.175.186:3000) to view it in the browser.
...
In the WebStorm menu `Run` select `Edit Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `http://54.219.175.186:3000` into the URL field and save the configuration.
...
```js
  "proxy": "http://54.219.175.186:4000",
```

This way, when you `fetch('/api/todos')` in development, the development server will recognize that it’s not a static asset, and will proxy your request to `http://54.219.175.186:4000/api/todos` as a fallback. The development server will **only** attempt to send requests without `text/html` in its `Accept` header to the proxy.

Conveniently, this avoids [CORS issues](http://stackoverflow.com/questions/21854516/understanding-ajax-cors-and-security-considerations) and error messages like this in development:

```
Fetch API cannot load http://54.219.175.186:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://54.219.175.186:3000' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
...
This shouldn’t affect you when developing on `54.219.175.186`, but if you develop remotely like [described here](https://github.com/facebookincubator/create-react-app/issues/2271), you will see this error in the browser after enabling the `proxy` option:
...
[does not apply to `54.219.175.186`](http://stackoverflow.com/questions/34160509/options-for-testing-service-workers-via-http/34161385#34161385).
...
HOST | :white_check_mark: | :x: | By default, the development web server binds to `54.219.175.186`. You may use this variable to specify a different host.
