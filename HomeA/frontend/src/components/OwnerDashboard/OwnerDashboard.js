...
class ShowProperty extends Component {
    ...
    componentDidMount(){
     ...
     // console.log(data)
      ...
      // });
      axios.get('http://54.219.175.186:3001/getownerproperty',{ params: { data: data}})
              .then((response) => {
                //console.log("Status Code : ",response.data);
              //   if(response.data === 400){
...
>
              ...
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <Route render={({ history}) => (
            <Button variant="contained" color="secondary" value ="Owner" onClick={() => { history.push({
      pathname:"/message",
      state:{
        ownerkey: this.state.ownerkey,
